...
func buildUpdateGround(prj *project.Project, prop []*project.Prop) (*UpdateGround, error) {
	var m UpdateGround
	...
	for _, p := range prop {
		...
		mesh := toVec3Array(prj.Mesh[p.Mesh].Position)
...
func buildUpdate(prj *project.Project) ([]*Update, error) {
	...
	for _, s := range prj.Scene {
		...
		u.Name = s.Name
		if len(s.Prop) > 0 {
			m, err := buildUpdateGround(prj, s.Prop)
			if err != nil {
				return nil, err
			}
